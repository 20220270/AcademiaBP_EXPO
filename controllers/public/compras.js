...
SEARCH_FORM.addEventListener('submit', (event) => {
  ...
  const fillTable = async (form = null) => {
    ...
    if (DATA.status) {
        ...
        // Agrupamos las compras por id_compra
        DATA.dataset.forEach(row => {
            if (!agrupadas[row.id_compra]) {
                agrupadas[row.id_compra] = [];
            }
            agrupadas[row.id_compra].push(row);
        });

        // Generamos las cards agrupadas
        //utiliza un bucle for...in, que se emplea para recorrer todas las propiedades enumerables de un objeto
        let rows = '';
        for (let id_compra in agrupadas) {
            rows += ` 
                <div class="row mb-4">
                    ${agrupadas[id_compra].map(row => `
                            ...
                            <div class="card-body">
                                ...
                                <div class="row mb-2">
                                    ...
                                    <div class="text-center mt-3">
                                        ...
                                        <button type="submit" class="btn mt-1 mostrarModalValoracion" onclick="openReport(${row.id_compra})">
