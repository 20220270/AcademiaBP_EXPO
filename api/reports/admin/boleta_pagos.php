    ...
    if ($boleta->setIdPago($_GET['idPagoARealizar'])) {
        ...
        if ($rowboleta = $boleta->readOne()) {
            ...
            if ($dataBoleta = $boleta->readBoletaPagos()) {
                ...
                $pdf->cell(90, 9, $pdf->encodeString('Fecha de pago: ' . $dataBoleta[0]['fecha_pago']), 'B', 0, 'L', 1);
                ...
                $pdf->cell(90, 9, $pdf->encodeString('Valor $: ' . $dataBoleta[0]['mensualidad_pagar'] . ' - ' . $dataBoleta[0]['numero_dias'] . ' días'), 'B', 1, 'L', 1);

                // Se continúan agregando las celdas restantes
                $pdf->cell(0, 11, $pdf->encodeString('Alumno: ' . $dataBoleta[0]['nombre'] . ' - Categoría ' . $dataBoleta[0]['categoria']), 'B', 1, 'L', 1);
                $pdf->cell(0, 11, $pdf->encodeString('Concepto: ' . $dataBoleta[0]['descripcion_pago']), 'B', 1, 'L', 1);
                $pdf->cell(0, 15, $pdf->encodeString('Próximo pago: ' . $dataBoleta[0]['fecha_proximo_pago']), 'B', 1, 'L', 1);
            } else {
